...
proc place_pmos {x_center y_center length nf index} {
	...
	for {set x [expr 23+$con_sep+29]} {$x+29<=$bx} {set x [expr $x+2*$con_sep+2*29]} {
	...
	for {set x 23} {$x+29<=$bx} {set x [expr $x+2*$con_sep+2*29]} {
		box [expr $x_center-$bx/2+$x] [expr $y_center-$by/2+40] [expr $x_center-$bx/2+$x+17]  [expr $y_center-$height_half_center]
		paint li
	}
	return $box_size
...
proc place_nmos {x_center y_center length nf index} {
	...
	### paint source connection
	for {set x [expr 6+$con_sep+29]} {$x+29<=$bx} {set x [expr $x+2*$con_sep+2*29]} {
		box [expr $x_center-$bx/2+$x] [expr $y_center+$by/2-40] [expr $x_center-$bx/2+$x+17]  [expr $y_center+$height_half_center]
		paint li
	}

    ### paint drain connection
	for {set x 6} {$x+29<=$bx} {set x [expr $x+2*$con_sep+2*29]} {
		box [expr $x_center-$bx/2+$x] [expr $y_center-$by/2+40] [expr $x_center-$bx/2+$x+17]  [expr $y_center-$height_half_center]
		paint li
	}
	return $box_size
